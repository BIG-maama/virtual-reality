...
public class SceneConnectorFinal : MonoBehaviour
{
    ...
    ? new Vector3(
        _config.rope.pivotPoint.x,  // X = نفس الـ Pivot = 40.66
        canvasSurface.position.y,   // Y = 1
        _config.rope.pivotPoint.z   // Z = نفس الـ Pivot = 0.004
      )
    : new Vector3(40.66f, 1f, 0f);
    ...
    private void SetupParticleSystem()
    {
        ...
        // ParticleSystem احتياطي فقط (معطل)
    ...
    private void HandlePaint(float dt)
    {
        ...
        _emitter.CleanupDeadParticles();
        _painter.Update(dt);
    }

    private void UpdateParticleVisuals()
    {
        // ✅ GPU Instancing — يُستدعى من Update() لا FixedUpdate()
        _gpuRenderer?.UpdateAndDraw(_emitter.ActiveParticles);
    }
    ...
    private void CreateSplat(Vector3 pos, Color col, float speed)
    {
        ...
        mat.color = col;
        mr.material = mat;
        float r = Mathf.Clamp(speed * 0.015f, 0.03f, 0.2f);
        go.transform.position = new Vector3(pos.x, _canvasYDynamic + 0.01f, pos.z);
        go.transform.localScale = new Vector3(r, 0.01f, r);
    ...
    public void SetPaintType(int typeIndex)
    {
        if (_config == null) return;
        _config.paint.paintType = (PaintType)Mathf.Clamp(typeIndex, 0, 3);
    }
    ...
    {
        if (_config == null) return;
        _config.paint.initialHeight = Mathf.Clamp(heightM, 0.01f, 0.30f);
        Restart();
    }

    public void SetHoles(System.Collections.Generic.List<HoleData> holes)
    {
        if (_config == null || holes == null || holes.Count == 0) return;
        _config.bucket.holes = holes;
    }

    public void SetFlowRate(float rate)
        => _emitter?.SetEmitRate(rate);

    public void SetDropletRadius(float radiusM)
        => _emitter?.SetDropletRadius(radiusM);
    ...
    {
        if (_config == null || colors == null || colors.Length == 0) return;
        _config.paint.colors = colors;
        paintColor = colors[0];
        _gpuRenderer?.SetColor(colors[0]);
    }
}